...
function draw() {
  //trex.debug = true;
  background(255);
  text("Pontuação: "+ score, windowWidth-300,windowHeight-300);
  
  if (gameState===PLAY){
    score = score + Math.round(getFrameRate()/60);
    ground.velocityX = -(6 + 3*score/100);
  
    if(keyDown("space") && trex.y >= windowHeight-120) {
      trex.velocityY = -12;
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(windowWidth,windowHeight-150,40,10);
    cloud.y = Math.round(random(windowHeight-300,windowHeight-200));
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(windowWidth,height-110,10,40);
    //obstacle.debug = true;
